...
def env(tmp_path, monkeypatch):
    ...
    conn.executemany(
        ...
        [
            (GROUP, "Wanderwochenende", "2026-01-03T10:00:00"),
...
def test_add_single_match_writes_whitelist(env):
    rc = whitelist_cli.main(["add", "wander", "-y"])
    assert rc == 0
...
def test_remove_drops_entry(env):
    whitelist_cli.main(["add", "wander", "-y"])
    rc = whitelist_cli.main(["remove", "wander", "-y"])
    assert rc == 0
    assert _allowed(env) == []
...
def test_list_shows_current_whitelist_with_names(env, capsys):
    ...
    assert rc == 0
    assert GROUP in out and "Wanderwochenende" in out
...
def test_chats_marks_whitelisted(env, capsys):
    ...
    out = capsys.readouterr().out
    assert rc == 0
    assert "Wanderwochenende" in out and "Anna" in out  # lists all
